...
class AtQueryDockWidget(QtWidgets.QDockWidget):
    ...
    def _setup_chat_view(self):
        ...
        <table width="100%" cellspacing="0" cellpadding="0" border="0" style="margin-bottom: 10px;">
            <tr>
                <td width="60" valign="top" align="left">
                    <img src='file://{self.logo_path}' width='50' height='50'>
                </td>
                <td valign="top" align="right" style="line-height: 1.2;">
        ...
        self.user_input.setStyleSheet(
            "border-radius: 15px; padding: 6px 12px; border: 1px solid #ccc; background-color: white;"
        )
    ...
    def _send_query(self):
        ...
        user_bubble = f"""
        <table width="100%" cellspacing="0" cellpadding="0" border="0" style="margin-top: 6px; margin-bottom: 6px;">
            <tr>
                <td width="20%"></td>
                <td style="background-color: #D6EAF8; color: #333333; border-radius: 15px; padding: 10px 14px;">
                    <b>You:</b> {user_text}
                </td>
            </tr>
        </table>
        ...
        self.chat_display.append(
            ...
            "<span style='background-color: #E0E0E0; color: #555555; "
            "padding: 3px 14px; border-radius: 12px; font-size: 11px;'>"
            "AtQuery is thinking...</span></div>"
        )
            ...
            for step in range(5):
                ...
                for tc in ai_msg["tool_calls"]:
                    ...
                    if tool_name == "load_toolbox_skills":
                        ...
                        self.chat_display.append(
                            f"<div align='center' style='margin: 4px 0;'>"
                            f"<span style='background-color: #E0E0E0; color: #555555; "
                            f"padding: 3px 14px; border-radius: 12px; font-size: 11px;'>"
                            f"🔧 Used tool: <code>{tool_name}</code></span></div>"
                        )
    ...
    def _show_fallback_card_in_chat(self, query: str, current_turn_history: list, aborted=False):
        ...
        <table width="100%" cellspacing="0" cellpadding="0" border="0" style="margin-top: 6px; margin-bottom: 6px;">
            <tr>
                <td width="5%"></td>
                <td style="background-color: #FFF8E1; color: #000; border-radius: 15px; border-left: 6px solid #FFC107; padding: 12px 14px;">
                    <b>{title}</b><br>
                    {subtitle}<br><br>
                    What would you like to do next?<br><br>
                    &nbsp;&nbsp;
                    <a href="atquery://best-match" style="background:#4CAF50; color:white; padding:5px 12px; border-radius:12px; text-decoration:none;">⚡ Execute Best Match</a>
                    &nbsp;&nbsp;
                    <a href="atquery://learn" style="background:#2196F3; color:white; padding:5px 12px; border-radius:12px; text-decoration:none;">🔍 Search &amp; Learn New Skill</a>
                </td>
                <td width="5%"></td>
            </tr>
        </table>
    ...
    def _handle_no_skill_fallback(self, query: str, current_turn_history: list, force_yes=False, force_no=False):
        ...
        if force_yes:
            ...
            self.chat_display.append(
                ...
                "<span style='background-color: #E0E0E0; color: #555555; "
                "padding: 3px 14px; border-radius: 12px; font-size: 11px;'>"
                "⚡ Force-loading all toolboxes...</span></div>"
            )
            QtWidgets.QApplication.processEvents()
            ...
            self.chat_display.append(
                ...
                "<span style='background-color: #E0E0E0; color: #555555; "
                "padding: 3px 14px; border-radius: 12px; font-size: 11px;'>"
                "🔍 Synthesizing a new skill...</span></div>"
            )
            QtWidgets.QApplication.processEvents()
            ...
            if tool_data:
                ...
                <table width="100%" cellspacing="0" cellpadding="0" border="0" style="margin-top:6px;margin-bottom:6px;">
                    <tr>
                        <td width="10%"></td>
                        <td style="background-color:#FFF3E0;color:#000;border-radius:15px;padding:12px 14px;">
                ...
                try:
                    ...
                    if impl_code:
                        ...
                        exec(impl_code, exec_context)  # nosec
    ...
    def handle_ai_response(self, text, suggested=None):
        ...
        <table width="100%" cellspacing="0" cellpadding="0" border="0" style="margin-top: 6px; margin-bottom: 6px;">
            <tr>
                <td style="background-color: #D5F5E3; color: #333333; border-radius: 15px; padding: 10px 14px;">
                    💡 <b>AtQuery:</b> {text}
                    <hr style="border: none; border-top: 1px solid rgba(0,0,0,0.1); margin: 8px 0;">
                    <div style="font-size: 11px; color: #555555;">
                        Not working? Try: &nbsp;
                        <a href="atquery://best-match" style="color: #2E7D32; text-decoration: none; font-weight: bold;">⚡ Force Match</a> &nbsp;&nbsp;
                        <a href="atquery://learn" style="color: #1565C0; text-decoration: none; font-weight: bold;">🔍 Search &amp; Learn</a>
                    </div>
                </td>
                <td width="20%"></td>
            </tr>
